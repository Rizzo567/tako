...
function GlassIconBtn({ img, onClick, badge, label, text, idle, clickKf, imgOrigin, fallback, imgScale }) {
  const [imgErr, setImgErr] = aS(false);
  return (
    <button onClick={(e) => { const im = e.currentTarget.querySelector('img'); if (im && clickKf) { try { const s = imgScale || 1; const kf = clickKf.map(f => ({ ...f, transform: `scale(${s}) ${f.transform}` })); im.animate(kf, { duration: 850, easing: 'ease-in-out' }); } catch (_) {} } onClick && onClick(); }} aria-label={label} className="press-sm"
      ...
      <span style={{ position: 'relative', width: 46, height: 46, display: 'grid', placeItems: 'center' }}>
        {imgErr && fallback ? (
          <span style={{ width: 40, height: 40, borderRadius: 14, background: 'var(--brand-tint)', border: '1.5px solid var(--brand)', display: 'grid', placeItems: 'center', fontSize: 13, fontWeight: 800, color: 'var(--brand-deep)', letterSpacing: '.02em' }}>{fallback}</span>
        ) : (
        <img src={img} alt="" onError={() => setImgErr(true)} style={{ width: 42, height: 42, objectFit: 'contain', display: 'block', animation: idle, transform: imgScale ? `scale(${imgScale})` : 'none', transformOrigin: imgOrigin || '50% 85%', filter: 'drop-shadow(0 5px 7px rgba(42,31,26,.24))' }} />
        )}
        {badge > 0 && (
...
function Header({ onCart, onWaiter, onLang, count, compact }) {
  return (
    <header style={{ flex: 'none', overflow: 'hidden', background: 'var(--surface)', position: 'relative', zIndex: 30,
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 36, padding: 'calc(var(--safe-t) + 8px) 14px 12px',
        ...
        <GlassIconBtn img="assets/nav/m-bag.png" onClick={onCart} badge={count} label="Carrello" text="Carrello" clickKf={KF_HOP} imgOrigin="50% 85%" />
        <GlassIconBtn img="assets/nav/m-lang.png" onClick={onLang} label="Lingua" text="Language" clickKf={KF_WIGGLE} imgOrigin="50% 50%" imgScale={1.5} fallback={(window.TAKO_LANG || 'it').toUpperCase()} />
      </div>
    </header>
  );
}
...
function LanguageSheet({ open, onClose }) {
  const cur = window.TAKO_LANG || 'it';
  const pick = (code) => {
    if (code === cur) { onClose(); return; }
    try { const u = new URL(location.href); u.searchParams.set('lang', code); location.href = u.toString(); } catch (e) { onClose(); }
  };
  return (
    <Sheet open={open} onClose={onClose} title={tr('language')} sub={tr('languageSub')}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, padding: '4px 18px calc(var(--safe-b) + 18px)' }}>
        {TAKO_LANGS.map(l => {
          const on = l.code === cur;
          return (
            <button key={l.code} onClick={() => pick(l.code)} className="press-sm"
              style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 14px', borderRadius: 'var(--r-card)', textAlign: 'left',
                background: on ? 'var(--brand-tint)' : 'var(--raised)', border: `1.5px solid ${on ? 'var(--brand)' : 'var(--hairline)'}`,
                boxShadow: on ? 'none' : 'var(--sh-1)' }}>
              <span style={{ width: 34, height: 34, borderRadius: 10, background: on ? 'var(--brand)' : 'var(--sunken)', color: on ? 'var(--on-brand)' : 'var(--ink-2)', display: 'grid', placeItems: 'center', fontSize: 12, fontWeight: 800, letterSpacing: '.03em' }}>{l.code.toUpperCase()}</span>
              <span style={{ flex: 1, fontSize: 15.5, fontWeight: on ? 700 : 600, color: 'var(--ink)' }}>{l.name}</span>
              {on && <Icon name="check" size={18} style={{ color: 'var(--brand-deep)' }} />}
            </button>
          );
        })}
      </div>
...
function App() {
  ...
  const [langOpen, setLangOpen] = aS(false);
  ...
  const inner = (
    <div className="app" style={bare ? { position: 'relative', '--safe-t': (SP.get('safet') || '40') + 'px', '--safe-b': '14px' } : { position: 'relative', '--safe-t': '52px', '--safe-b': '22px' }}>
      {!entered ? (
        <Splash onEnter={() => setView('menu')} />
      ) : (
        <>
          <Header count={count} compact={navCompact} onCart={() => setCartOpen(true)} onWaiter={() => setWaiterOpen(true)} onLang={() => setLangOpen(true)} />
