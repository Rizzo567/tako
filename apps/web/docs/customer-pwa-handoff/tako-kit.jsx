...
/* burst di coriandoli a tutto schermo (WAAPI, si autodistrugge) */
function confettiBurst(x, y, n = 26) {
  ...
  const cx = x != null ? x : innerWidth / 2, cy = y != null ? y : innerHeight * 0.62;
  for (let i = 0; i < n; i++) {
    ...
    const an = p.animate([
      ...
      { transform: `translate(${dx}px,${dy}px) rotate(${rot * 0.5}deg) scale(1)`, opacity: 1, offset: .45 },
...
Object.assign(window, { Icon, IconBtn, Btn, Qty, Sheet, Toaster, toast, dismissToast, Confetti, confettiBurst, AllergenDot, Tag, useReducedMotion, useMountTransition, flyToCart, bounceCart, useCountUp, Bubbles });
