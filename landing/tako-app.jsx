...
/* ───────────────── header ───────────────── */
function Header({ onCart, onWaiter, count, table }) {
    ...
    <>
      <header style={{ flex: 'none', display: 'flex', alignItems: 'center', gap: 11, padding: 'calc(var(--safe-t) + 12px) 14px 11px', background: 'var(--surface)', position: 'relative', zIndex: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <p style={{ fontSize: 12.5, color: 'var(--ink-2)', fontWeight: 600, marginTop: 1 }}>{tr('table')} {table != null ? table : s.tableNumber}</p>
        </div>
  ...
  const inner = (
    <div className="app" style={bare ? { '--safe-t': '40px', '--safe-b': '14px' } : { '--safe-t': '52px', '--safe-b': '22px' }}>
      {!entered ? (
        ...
        <>
          <Header count={count} table={tableNo} onCart={() => setCartOpen(true)} onWaiter={() => setWaiterOpen(true)} />
