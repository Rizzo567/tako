...
/* ═══════════════════ SALA LIVE ═══════════════════ */
function ScreenSala({ mobile, rooms, calls, onSetTableState, onResolveCall }) {
          ...
          <div style={{ display: "flex", flexDirection: mobile ? "column" : "row", gap: 1, background: "var(--hairline)" }}>
            {calls.map((c, i) => (
              <div key={i} style={{ flex: 1, display: "flex", alignItems: "center", gap: 10, padding: "11px 16px", background: "var(--raised)" }}>
                ...
                <Btn size="sm" kind="soft" onClick={() => { onResolveCall && onResolveCall(i); toast(`Chiamata T${c.tavolo} risolta`, { type: "success" }); }}>Risolvi</Btn>
              </div>
            ))}
          </div>
        </Card>
  ...
  return (
    <ScreenScroll mobile={mobile}>
      <PageHead mobile={mobile} tako="pay" title="Cassa" sub={`${bills.length} conti aperti`}
        actions={!mobile && <Btn kind="brand" icon="plus" onClick={() => toast("Nuovo conto creato", { type: "success" })}>Nuovo conto</Btn>} />
  ...
  return (
    <div style={{ width: mobile ? 354 : 720, maxWidth: "100%", maxHeight: mobile ? "calc(100% - 48px)" : "100%", background: "var(--surface)", borderRadius: mobile ? 24 : "var(--r-xl)", boxShadow: "var(--sh-pop)", display: "flex", flexDirection: mobile ? "column" : "row", overflow: mobile ? "auto" : "hidden" }}>
      ...
      <div style={{ flex: 1, padding: mobile ? "16px 18px" : 22, display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        <Btn kind="ok" size={mobile ? "md" : "lg"} full icon="check" style={{ marginTop: mobile ? 10 : 14 }} onClick={onDone} disabled={method === "contanti" && (parseFloat(given) || 0) < total}>
    ...
    <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column", paddingTop: mobile ? 104 : 0 }}>
      ...
      {items.length > 0 && (
        <div style={{ flex: "none", padding: mobile ? "14px 14px 92px" : "16px 26px", borderTop: "1px solid var(--hairline)", background: "var(--raised)", display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <Btn kind="brand" size="lg" icon="check" onClick={() => { setCart({}); setTavolo(null); toast(`Comanda T${tavolo} inviata in cucina`, { type: "success", icon: "kitchen" }); }}>Invia comanda</Btn>
        </div>
      )}
    </div>
  );
...
function ScreenTavoli({ mobile }) {
  const all = ROOMS.flatMap(r => r.tables.map(t => ({ ...t, room: r.nome })));
  return (
    <ScreenScroll mobile={mobile}>
      <PageHead mobile={mobile} title="Gestione Tavoli" sub={`${all.length} tavoli · ${ROOMS.length} sale`} actions={<Btn kind="brand" icon="plus" onClick={() => toast("Aggiungi tavolo")}>Nuovo tavolo</Btn>} />
      <Card pad={0} style={{ overflow: "hidden" }}>
        {all.map((t, i) => (
          <div key={t.n} style={{ display: "flex", alignItems: "center", gap: 14, padding: "13px 16px", borderBottom: i < all.length - 1 ? "1px solid var(--hairline)" : "none" }}>
            ...
            <IconBtn name="edit" tone="ghost" onClick={() => toast("Modifica tavolo " + t.n)} />
            <IconBtn name="trash" tone="ghost" style={{ color: "var(--danger)" }} onClick={() => toast("Elimina tavolo " + t.n)} />
          </div>
        ))}
      </Card>
    </ScreenScroll>
...
function ScreenQR({ mobile }) {
  ...
  return (
    <ScreenScroll mobile={mobile}>
      <PageHead mobile={mobile} tako="phone" title="QR Codes" sub="Genera e scarica i QR dei tavoli" actions={<Btn kind="brand" icon="download" onClick={() => toast("QR scaricati (ZIP)", { type: "success" })}>Scarica tutti</Btn>} />
      <div style={{ display: "grid", gridTemplateColumns: mobile ? "repeat(2,1fr)" : "repeat(auto-fill,minmax(150px,1fr))", gap: 14 }}>
        {all.map(t => (
          <Card key={t.n} pad={16} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
            ...
            <Btn size="sm" kind="soft" full icon="download" onClick={() => toast(`QR tavolo ${t.n} scaricato`)}>Scarica</Btn>
          </Card>
        ))}
      </div>
    </ScreenScroll>
