...
function ItemSheet({ item, open, onClose }) {
  ...
  const add = () => {
    ...
    onClose();
    toast(`${qty}× ${item.name} aggiunto`, { type: 'success' });
  };
    ...
    <Sheet open={open} onClose={onClose} maxH="92%">
      <div style={{ padding: '0 0 18px' }}>
        ...
        <div style={{ padding: '0 20px' }}>
          <div ref={heroRef} className="dish-img" style={{ width: '100%', height: 188, borderRadius: 20, ...dishStyle(item.name), animation: 'pop-soft .55s var(--spring)' }}>
            ...
            {item.tags?.length > 0 && (
              <div style={{ position: 'absolute', top: 12, left: 12, display: 'flex', gap: 6 }}>{item.tags.map(t => <Tag key={t}>{t}</Tag>)}</div>
            )}
          </div>
        </div>

        <div style={{ padding: '18px 20px 0' }}>
          <h3 style={{ fontSize: 23, fontWeight: 700 }}>{item.name}</h3>
          <p style={{ color: 'var(--ink-2)', fontSize: 14.5, lineHeight: 1.5, marginTop: 7 }}>{item.desc}</p>

          {item.allergens.length > 0 && (
            <div style={{ marginTop: 16 }}>
              <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase', color: 'var(--ink-3)', marginBottom: 8 }}>Allergeni</p>
          ...
          {item.variants && (
            <div style={{ marginTop: 20 }}>
              <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase', color: 'var(--ink-3)', marginBottom: 8 }}>Scegli la porzione</p>
                ...
                {item.variants.map(opt => {
                  ...
                  return (
                    <button key={opt.id} className="press-sm" onClick={() => setVariant(opt.id)}
                      ...
                      <span style={{ fontWeight: 700, fontSize: 14, color: opt.delta ? 'var(--brand-deep)' : 'var(--ink-3)' }}>{opt.delta ? '+ ' + eur(opt.delta) : 'incluso'}</span>
                    </button>
                  );
                })}
              </div>
          ...
          <div style={{ marginTop: 20 }}>
            <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase', color: 'var(--ink-3)', marginBottom: 8 }}>Note per la cucina</p>
            <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Es. senza cipolla, ben cotto…" rows={2}
      ...
      <div style={{ position: 'sticky', bottom: 0, background: 'var(--raised)', borderTop: '1px solid var(--hairline)', padding: '12px 20px', display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        <Btn full onClick={add} style={{ flex: 1 }}>
          <span style={{ flex: 1, textAlign: 'left' }}>Aggiungi</span>
...
function FilterSheet({ open, onClose, excluded, setExcluded }) {
  ...
  return (
    <Sheet open={open} onClose={onClose} title="Filtra per allergeni" sub="Nascondi i piatti che li contengono" leadIcon="filter">
      ...
      <div style={{ position: 'sticky', bottom: 0, background: 'var(--raised)', borderTop: '1px solid var(--hairline)', padding: '12px 20px', display: 'flex', gap: 10 }}>
        {excluded.length > 0 && <Btn kind="ghost" onClick={() => setExcluded([])}>Azzera</Btn>}
        <Btn full onClick={onClose} style={{ flex: 1 }}>{excluded.length ? `Mostra piatti` : 'Fatto'}</Btn>
      </div>
    </Sheet>
  );
}
    ...
    <div ref={scrollRef} onScroll={onScroll} className="scroll" style={{ flex: 1, minHeight: 0 }}>
            ...
            return (
              <button key={s.id} ref={el => tabRefs.current[s.id] = el} onClick={() => goTo(s.id)} className="press-sm"
                style={{ position: 'relative', zIndex: 1, flex: 'none', padding: '8px 15px', borderRadius: 999, fontSize: 14, fontWeight: 700, whiteSpace: 'nowrap',
                  background: 'transparent', color: on ? 'var(--surface)' : 'var(--ink-3)', transition: 'color .3s var(--out)' }}>{s.name}</button>
            );
          })}
      ...
      {excluded.length > 0 && (
        <div style={{ margin: '0 16px 4px', padding: '9px 14px', borderRadius: 12, background: 'var(--brand-wash)', display: 'flex', alignItems: 'center', gap: 8, fontSize: 12.5, color: 'var(--brand-deep)', fontWeight: 600 }}>
          <Icon name="info" size={15} /> Nascondo i piatti con {excluded.map(e => ALLERGEN_MAP[e].label.toLowerCase()).join(', ')}.
        </div>
      )}
      ...
      <div style={{ padding: '4px 16px 0' }}>
        {visibleSections.map((s, si) => (
          <section key={s.id} ref={el => secRefs.current[s.id] = el} style={{ paddingTop: si === 0 ? 6 : 22 }}>
            <h2 className="serif" style={{ fontSize: 27, marginBottom: 12, paddingLeft: 2 }}>{s.name}</h2>
              ...
              {s.items.map((it, i) => (
                <button key={it.id} onClick={() => openItem(it)} disabled={!it.available} className={it.available ? 'press-sm' : ''}
                  ...
                  <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
                    ...
                    <p style={{ color: 'var(--ink-2)', fontSize: 13, lineHeight: 1.45, marginTop: 4, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{it.desc}</p>
                  ...
                  <div style={{ position: 'relative', flex: 'none' }}>
                    <Dish name={it.name} size={86} radius={14} />
                    {!it.available
                      ? <span style={{ position: 'absolute', inset: 0, borderRadius: 14, background: 'rgba(42,31,26,.66)', backdropFilter: 'saturate(.4)', color: '#fff', display: 'grid', placeItems: 'center', fontSize: 11.5, fontWeight: 700, letterSpacing: '.03em' }}>Esaurito</span>
      ...
      <div style={{ position: 'absolute', left: 16, right: 16, bottom: 'calc(var(--safe-b) + 84px)', zIndex: 30, pointerEvents: count ? 'auto' : 'none',
        ...
        <button onClick={onOpenCart} className="press"
          ...
          <span style={{ fontWeight: 700, fontSize: 16, flex: 1, textAlign: 'left' }}>Vedi carrello</span>
...
function CartSheet({ open, onClose, onConfirm }) {
  ...
  return (
    <Sheet open={open} onClose={onClose} title="Il tuo ordine" sub={items.length ? `Tavolo ${sessionStore.get().tableNumber}` : null} leadIcon="bag" maxH="90%">
      {items.length === 0 ? (
        <div style={{ padding: '20px 30px 40px', textAlign: 'center' }}>
          ...
          <h3 style={{ fontSize: 19, fontWeight: 700, marginTop: 8 }}>Il carrello è vuoto</h3>
          <p style={{ color: 'var(--ink-2)', fontSize: 14, marginTop: 6, marginBottom: 20 }}>Aggiungi qualcosa di buono dal menù.</p>
          <Btn onClick={onClose} icon="arrowLeft">Torna al menù</Btn>
        </div>
      ) : (
        <>
          <div style={{ padding: '4px 20px 8px', display: 'flex', flexDirection: 'column', gap: 10 }}>
            {items.map(li => (
              <div key={li.key} style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '10px 0', borderBottom: '1px solid var(--hairline)', animation: 'rise-fade .35s var(--out) both' }}>
                <Dish name={li.name} size={52} radius={12} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontWeight: 600, fontSize: 14.5, lineHeight: 1.3 }}>{li.name}</p>
            ...
            <div style={{ marginTop: 6 }}>
              <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Note per tutto l'ordine (es. portate insieme)" rows={2}
          ...
          <div style={{ position: 'sticky', bottom: 0, background: 'var(--raised)', borderTop: '1px solid var(--hairline)', padding: '12px 20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 10 }}>
              <span style={{ color: 'var(--ink-2)', fontWeight: 600 }}>Totale</span>
              <span className="serif tnum" style={{ fontSize: 26 }}>{eur(animTotal)}</span>
            </div>
            <Btn full onClick={onConfirm} icon="check">Conferma ordine</Btn>
          </div>
        </>
      )}
    </Sheet>
