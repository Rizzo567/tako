    ...
    return (
      <div className="scroll" style={{ flex: 1, display: 'grid', placeItems: 'center', padding: 30, textAlign: 'center' }}>
        <div>
          ...
          <h3 style={{ fontSize: 20, fontWeight: 700, marginTop: 10 }}>{tr('noActiveOrder')}</h3>
          <p style={{ color: 'var(--ink-2)', fontSize: 14, marginTop: 6, marginBottom: 20 }}>{tr('noActiveOrderSub')}</p>
          <Btn onClick={goMenu} icon="bowl">{tr('goToMenu')}</Btn>
        </div>
      </div>
    );
  }
    ...
    <div className="scroll" style={{ flex: 1, padding: '8px 16px calc(var(--safe-b) + 96px)' }}>
      ...
      <div key={o.status} style={{ position: 'relative', overflow: 'hidden', borderRadius: 'var(--r-card)', padding: '20px 18px', marginTop: 4,
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ flex: 1 }}>
            <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase', opacity: .8 }}>{tr('orderWord')} {o.orderId}</p>
            <h2 className="serif" style={{ fontSize: 28, lineHeight: 1, marginTop: 3 }}>{trStep(step).label}</h2>
            <p style={{ fontSize: 14, opacity: .92, marginTop: 4 }}>{trStep(step).desc}</p>
          </div>
        </div>
      </div>
          ...
          return (
            <div key={s.id} style={{ display: 'flex', gap: 14, position: 'relative' }}>
              ...
              <div style={{ paddingBottom: 18, paddingTop: 3, opacity: done || on ? 1 : .5, transition: 'opacity .4s' }}>
                <p style={{ fontWeight: 700, fontSize: 15.5 }}>{trStep(s).label}</p>
                <p style={{ color: 'var(--ink-2)', fontSize: 13, marginTop: 1 }}>{trStep(s).desc}</p>
              </div>
            </div>
          );
        })}
      ...
      <div style={{ background: 'var(--raised)', borderRadius: 'var(--r-card)', padding: '16px 18px', marginTop: 12, boxShadow: 'var(--sh-1)' }}>
        <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase', color: 'var(--ink-3)', marginBottom: 12 }}>{tr('summary')}</p>
        {o.items.map(li => (
          <div key={li.key} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 11 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ fontWeight: 600, fontSize: 14.5 }}>{trLine(li)}</p>
        ...
        {o.notes && <p style={{ fontSize: 12.5, color: 'var(--ink-2)', fontStyle: 'italic', borderTop: '1px solid var(--hairline)', paddingTop: 10 }}>{tr('notePre')} {o.notes}</p>}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', borderTop: '1px solid var(--hairline)', paddingTop: 12, marginTop: 4 }}>
          <span style={{ fontWeight: 700 }}>{tr('total')}</span>
          <span className="serif tnum" style={{ fontSize: 23 }}>{eur(animTotal)}</span>
        </div>
      </div>

      <div style={{ marginTop: 14 }}><Btn full kind="soft" onClick={goMenu} icon="plus">{tr('addMore')}</Btn></div>
    </div>
  );
}
...
/* ───────────────── WAITER CALL ───────────────── */
const WAITER_OPTS = [
  { kind: 'help', icon: 'help', labelKey: 'waiterHelp', subKey: 'waiterHelpSub' },
  { kind: 'bill', icon: 'receipt', labelKey: 'waiterBill', subKey: 'waiterBillSub' },
  { kind: 'water', icon: 'droplet', labelKey: 'waiterWater', subKey: 'waiterWaterSub' },
];
function WaiterSheet({ open, onClose }) {
  const [sent, setSent] = sS(null);
  sE(() => { if (open) setSent(null); }, [open]);
  const call = (opt) => {
    setSent({ ...opt, label: tr(opt.labelKey) }); // POST /customer/waiter-call {type}
    setTimeout(() => { onClose(); }, 2100);
  };
  return (
    <Sheet open={open} onClose={onClose} title={sent ? null : tr('callWaiter')} sub={sent ? null : `${tr('table')} ${sessionStore.get().tableNumber}`} leadIcon={sent ? null : 'bell'}>
      {sent ? (
        <div style={{ position: 'relative', padding: '14px 30px 40px', textAlign: 'center' }}>
          ...
          <h3 style={{ fontSize: 21, fontWeight: 700, marginTop: 16 }}>{tr('waiterDone')}</h3>
          <p style={{ color: 'var(--ink-2)', fontSize: 14.5, marginTop: 6 }}>{sent.label} — {tr('waiterComing')} {sessionStore.get().tableNumber}.</p>
        </div>
      ) : (
        <div style={{ padding: '4px 20px 22px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          {WAITER_OPTS.map((o, i) => (
            <button key={o.kind} data-kind={o.kind} className="press-sm" onClick={() => call(o)}
              ...
              <span style={{ flex: 1 }}>
                <span style={{ display: 'block', fontWeight: 700, fontSize: 16 }}>{tr(o.labelKey)}</span>
                <span style={{ display: 'block', color: 'var(--ink-2)', fontSize: 13, marginTop: 1 }}>{tr(o.subKey)}</span>
              </span>
